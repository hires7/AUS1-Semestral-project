...
namespace ds::adt {
    ...
    bool SequenceTable<K, T, SequenceType>::tryFind(const K& key, T*& data) const
    {
        ...
        //data = &blockWithKey->data_.data_; dole logickejsie vysvetlne
		TableItem<K, T>* tableItem = &blockWithKey->data_;
		T& dataRef = tableItem->data_;
		data = &dataRef;
        return true;
    }
